...
process trim_reads {
    ...
    script:
    """
    /mnt/software/bin/seqtk trimfq  -b ${params.trim_length} ${fastq1} | pigz -c -p ${task.cpus} > ${name}.R1.trimmed.fq.gz
    /mnt/software/bin/seqtk trimfq  -b ${params.trim_length} ${fastq2} | pigz -c -p ${task.cpus} > ${name}.R2.trimmed.fq.gz
...
process mutation_calling {
    ...
    input:
    set val(name), file(mapped_sam_file) from mapped_sam_for_mutation
...
def take_first_item(condition, sep="--") {
...
def remove_last_item(condition, sep="_") {
    // remove the batch name at the end
